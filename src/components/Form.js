...
<>
  <h2 className="text-center pb-2.5">{submitMessage}</h2>
   <form className="flex flex-col items-center" onSubmit={handleSubmit(_handleSubmit)}>
      ...
      <input className="w-[300px] sm:w-2/3 lg:w-1/3 uppercase text-center appearance-none border rounded leading-tight focus:outline-none focus:shadow-outline" placeholder="name" value={mailchimp.FNAME} {...register("FNAME")}  name="FNAME" onChange={_handleChange}/>
        <p className="text-center text-red-700 py-[5px]">
            {errors.FNAME?.message}
        </p>
      {/* include validation with required or other standard HTML validation rules */}
      <input className="w-[300px] sm:w-2/3 lg:w-1/3 uppercase text-center appearance-none border rounded leading-tight focus:outline-none focus:shadow-outline" placeholder='email' value={mailchimp.EMAIL} {...register("EMAIL")} name="EMAIL" onChange={_handleChange}/>
     
      <p className="py-[5px] text-center text-red-700">{errors.EMAIL?.message}</p>
    
<button className="uppercase text- mb-2.5 cursor-pointer bg-black hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit" disabled={!isValid}>
    submit
</button>
    </form>
    </>
