...
const Header = () => {
  ...
  return (
   ...
   <header className='sticky top-0 z-50 bg-white'>
    ...
    {/* left */}
    <Link to='/' className='ml-5 select-none font-semibold w-36' onClick={handleClose}> 
        Mishean
    </Link>

    <Link to='/work' className='font-semibold hidden md:flex justify-center'> 
        Work
    </Link>
        
     {/* right */}
    <Link to='/subscribe' className='font-semibold hidden md:flex w-36 mr-5'> 
        Subscribe
    </Link>
    ...
    <div className={nav ? 'overflow-y-scroll justify-center z-10 md:hidden ease-in duration-300 fixed top-0 bottom-0 left-0 right-0 bg-white/90 w-screen flex flex-col items-center': 'flex fixed top-0 h-screen left-[-100%] ease-in duration-300'}>
            <Link className='select-none p-5 text-xl font-semibold'  to='/work' onClick={handleClick}>Work</Link>
            <Link className='select-none p-5 text-xl font-semibold' to='/subscribe' onClick={handleClick}>Subscribe</Link>
    </div> 
   </>
  )
}
